...
int fsm_init(fsm_t **f, int rfd, int wfd) {
    ...
    // 关键：强制设置为非阻塞模式
    int r_flags = fcntl(rfd, F_GETFL); // FIXME获取文件描述符rfd所指文件的文件状态标志
    r_flags |= O_NONBLOCK; // 加上非阻塞标志
    fcntl(rfd, F_SETFL, r_flags); // 设置读文件描述符为非阻塞模式
    
    int w_flags = fcntl(wfd, F_GETFL); // FIXME获取文件描述符wfd所指文件的文件状态标志
...
void fsm_drive(fsm_t *f) {
    ...
    switch (f->state) {
        ...
        case STATE_W: // 写状态：尝试从buf中写数据到wfd
            // FIXME续写逻辑：从 pos 位置开始写，剩余长度为 count - pos
